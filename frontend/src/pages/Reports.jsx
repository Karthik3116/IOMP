...
export default function Reports() {
  ...
  const analytics = useMemo(() => {
    ...
    if (filtered.length === 0) return { total: 0, filtered: [] };
    ...
    // 5. CHART DATA: CONFIDENCE BREAKDOWN (Pie)
    const confLevels = [
        { name: 'Critical (>80%)', value: 0, color: '#ef4444' }, // Red
        { name: 'High (60-80%)', value: 0, color: '#f97316' },   // Orange
        { name: 'Moderate (<60%)', value: 0, color: '#3b82f6' }  // Blue
    ];
    filtered.forEach(r => {
    ...
    // 6. CHART DATA: SCATTER (Time vs Confidence)
  ...
  // 🟢 CSV EXPORT
  const downloadCSV = () => {
    if (!analytics?.filtered.length) return;
    const headers = ["Timestamp,Camera,Class,Confidence,ImageID\n"];
    const rows = analytics.filtered.map(r => 
        `${new Date(r.timestamp).toISOString()},${r.cameraName},${r.detectedClass},${r.confidence},${r.image}`
    );
  ...
  const currentTableData = analytics?.filtered.slice(
      (currentPage - 1) * itemsPerPage, 
      currentPage * itemsPerPage
  );

  // Helper for unique cameras dropdown
  const uniqueCameras = useMemo(() => [...new Set(reports.map(r => r.cameraName))], [reports]);
        ...
        <div className="flex flex-wrap gap-3 bg-slate-800 p-2 rounded-lg border border-slate-700">
            ...
            <div className="relative">
                <select 
                    ...
                    className="bg-slate-900 text-white text-sm pl-8 pr-4 py-2 rounded border border-slate-700 focus:border-indigo-500 outline-none appearance-none cursor-pointer hover:bg-slate-700 transition"
                >
            ...
            <div className="relative">
                <select 
                    ...
                    className="bg-slate-900 text-white text-sm pl-8 pr-8 py-2 rounded border border-slate-700 focus:border-indigo-500 outline-none appearance-none cursor-pointer hover:bg-slate-700 transition"
                >
            ...
            <button 
                onClick={downloadCSV}
                className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded text-sm font-bold flex items-center gap-2 transition-colors"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 shadow-lg">
            ...
            <p className="text-xs text-red-400 mt-2 flex items-center gap-1">
                <TrendingUp size={12}/> High Traffic Time
            </p>
        </div>
        ...
        <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 shadow-lg">
            ...
            <h3 className="text-xl font-black text-white mt-2 truncate">
                {analytics.locationData[0]?.name || "None"}
            </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-lg h-80 flex flex-col">
            <h4 className="text-white font-bold mb-4 flex items-center gap-2 text-sm uppercase tracking-wider">
                <Calendar size={16} className="text-indigo-400"/> Temporal Frequency
            </h4>
        ...
        <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-lg h-80 flex flex-col">
             <h4 className="text-white font-bold mb-4 flex items-center gap-2 text-sm uppercase tracking-wider">
                <Activity size={16} className="text-emerald-400"/> AI Confidence Levels
            </h4>
            <div className="flex-1">
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                        <Pie
                            data={analytics.confLevels}
                            innerRadius={60}
                            outerRadius={80}
                            paddingAngle={5}
                            dataKey="value"
                        >
                            {analytics.confLevels.map((entry, index) => (
        ...
        {/* 3. SCATTER PLOT (Confidence vs Time) */}
        <div className="lg:col-span-2 bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-lg h-80 flex flex-col">
            <h4 className="text-white font-bold mb-4 flex items-center gap-2 text-sm uppercase tracking-wider">
                <Filter size={16} className="text-pink-400"/> Anomaly Detection (Time vs Confidence)
            </h4>
            <div className="flex-1">
                 <ResponsiveContainer width="100%" height="100%">
                    <ScatterChart>
        ...
        <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-lg h-80 flex flex-col">
             <h4 className="text-white font-bold mb-4 flex items-center gap-2 text-sm uppercase tracking-wider">
                <MapPin size={16} className="text-red-400"/> Threat Sources
            </h4>
            <div className="flex-1">
                <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="h-64 flex items-center justify-center bg-slate-800/50 rounded-xl border border-dashed border-slate-700">
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden shadow-2xl">
        <div className="p-4 border-b border-slate-700 flex justify-between items-center bg-slate-800">
            <h4 className="text-white font-bold flex items-center gap-2">
                <ShieldAlert size={18} className="text-slate-400"/> Detailed Log
            </h4>
            <span className="text-xs text-slate-500">
                Showing {currentTableData?.length || 0} of {analytics?.total || 0} Events
            </span>
        </div>
                    ...
                    <td className="p-4">
                        <div className="flex items-center gap-2">
                            <div className="w-24 bg-slate-700 rounded-full h-1.5 overflow-hidden">
                                <div 
                                    className={`h-full rounded-full ${report.confidence > 0.8 ? 'bg-red-500' : report.confidence > 0.5 ? 'bg-orange-500' : 'bg-blue-500'}`} 
                    ...
                    <td className="p-4 text-right">
                        {report.image && (
                            <button 
                                ...
                                className="text-indigo-400 hover:text-white bg-slate-900/50 hover:bg-indigo-600 p-2 rounded-lg transition-all border border-slate-700 hover:border-indigo-500 group"
                            >
          ...
          <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-50 flex items-center justify-center p-8" onClick={() => setSelectedImage(null)}>
              <div className="relative max-w-4xl w-full animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="absolute top-4 right-4 flex gap-2">
                      ...
                      <button onClick={() => setSelectedImage(null)} className="bg-red-600 text-white p-2 rounded-lg hover:bg-red-500 shadow-lg shadow-red-900/50 transition-colors">
                          ✕
                      </button>
                  </div>
              </div>
